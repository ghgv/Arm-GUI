...
QString str;
QLCDNumber *angleZ,*angleR,*angleL;
QPlainTextEdit *dlog;
float yPOS=0;
float xPOS=0;
float zPOS=0;
...

{
    ...
    connect(ui->UP, SIGNAL(pressed()),this, SLOT(LMCW( )));
    connect(ui->DOWN, SIGNAL(pressed()),this, SLOT(LMACW( )));

    connect(ui->RIGHT, SIGNAL(pressed()),this, SLOT(ZMACW( )));
    connect(ui->LEFT, SIGNAL(pressed()),this, SLOT(ZMCW( )));

    connect(ui->FRONT, SIGNAL(pressed()),this, SLOT(RMCW()));
    connect(ui->BACK, SIGNAL(pressed()),this, SLOT(RMACW()));
...
void MainWindow::calculate()// From the button to set an initial position
{
   ...
   str=ui->X->toPlainText(); //Read X in centimeter (cm)
   x=str.toDouble();

   str=ui->Y->toPlainText();//Read Y in cm
   y=str.toDouble();
   ...
   if(t>0){
       for(m1=0;m1<1.01;m1=m1+0.01)
       {
           ...
           n=-(3.1416-c-phi);
           xPOS=-beta*180/(3.1416); //Right Motor angle.Angles are transfered in degrees
           yPOS=-n*180/(3.1416);    //Left Motor angle
           zPOS=90-theta*180/3.1416; //Base angle
...
void MainWindow::python()
{
    ...
    const char pFile[] = "C:\\Users\\German\\Documents\\programacion\\test\\pyemb6.py";
    ...
    if(fp==NULL)
        ...
        try{
            PyRun_AnyFile(fp, pFile);  //Sometime in double click of the python file get an exception
            }
        catch(int x){
...
void MainWindow::RMCW(){//BACK button
     xPOS=xPOS-1;
     xMoverD();
}

void MainWindow::LMACW(){//UP
     yPOS=yPOS+1;
     yMoverD();
}

void MainWindow::LMCW(){//Down Motor
     yPOS=yPOS-1;
     yMoverU();
}

void MainWindow::ZMACW(){
    zPOS=zPOS+1;
    zMoverD();
}

void MainWindow::ZMCW(){
    zPOS=zPOS-1;
    qDebug()<<"Z";
    zMoverD();
}
